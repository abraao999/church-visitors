...
function civilInZone(date: Date, timeZone = CHURCH_TIMEZONE): CivilTime {
...
function civilToUtc(
  ...
  const shift = (instant: number): number => {
    const seen = civilInZone(new Date(instant));
...
export function startOfDay(date: Date): Date {
  const { year, month, day } = civilInZone(date);
  return civilToUtc(year, month, day, 0, 0, 0, 0);
}

export function endOfDay(date: Date): Date {
  const { year, month, day } = civilInZone(date);
  return civilToUtc(year, month, day, 23, 59, 59, 999);
}
